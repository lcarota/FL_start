...
from DEC_simple import ClusteringLayer, autoencoder
...
def cluster_acc(y_true, y_pred):
    ...
    # Return
        accuracy, in [0,1]"""
    #y_true=np.array([1,3,4,1,2])
    y_true = y_true.astype(np.int64)
    #y_pred=np.array([2,1,5,1,3])
...
class IDEC(object):
 ...
 #           exit()
    ...
    def extract_feature(self, x):  # extract features from before clustering layer
        encoder = Model(self.model.input, self.model.layers['encoder_%d' % (self.n_stacks - 1)].output)
...
#    parser.add_argument('dataset', default='mnist', choices=['mnist', 'usps', 'reutersidf10k'])
 ...
 #       x, y = load_reuters('data/reuters')
    ...
    idec.initialize_model(ae_weights='weights.h5', gamma=args.gamma, optimizer=optimizer)
